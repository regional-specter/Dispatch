...
import type { CargoPackage } from "./cargo-physics";

export function makeCanvasTexture(drawFn: (ctx: CanvasRenderingContext2D, w: number, h: number) => void, w = 512, h = 512) {
...
function roundedBox(w: number, h: number, d: number, radius: number, seg = 4) {
...
export interface UldMeshResult {
  group: THREE.Group;
  dims: { x: number; y: number; z: number };
  label: HTMLDivElement;
}

export function createUldMesh(pkg: CargoPackage): UldMeshResult {
  ...
  label.className = "cargo-pkg-label";
...
export function buildAircraftShell(
  hold: { lengthM: number; widthM: number; heightM: number },
  zones: Record<string, { xRange: readonly [number, number]; color: number }>,
  gridCellM: number,
) {
  ...
  for (const [, spec] of Object.entries(zones)) {
  ...
  const tail = new THREE.Mesh(new THREE.ConeGeometry(widthM * 0.42, 1.8, 32), shellMat.clone());
...
export async function loadEnvironment(scene: THREE.Scene, renderer: THREE.WebGLRenderer) {
